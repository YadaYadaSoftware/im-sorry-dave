...
### Requirement: Aspire-injected endpoint is the local target

When launched by the Aspire AppHost, the TUI SHALL use the injected API endpoint as its `local`
target (folding it in rather than adding a separate entry), so that starting the AppHost and picking
`local` in the TUI reaches the AppHost's API, and the only choice the operator makes is local vs the
remote target(s). A command-line selection SHALL still override this default.

#### Scenario: Launched by the AppHost

- **WHEN** the AppHost injects the API endpoint via service discovery and no command-line target is set
- **THEN** the TUI's `local` target points at the injected endpoint and is the active target

#### Scenario: Command-line selection overrides the default

- **WHEN** a target is chosen on the command line
- **THEN** the TUI uses that target even when an endpoint is injected
