...
- **`/post` summarizes since the last `/post` (rolling per-channel window).** The command is named **`/post`**, and its window is the messages posted **since the previous successful `/post`** in that channel — so consecutive posts cover only new conversation and never re-summarize already-posted content. A **per-channel cursor** (the timestamp/marker of the last successful `/post`) is stored and **advances only when a `/post` writes back successfully**; a no-op/rejected/failed post leaves the cursor put, so the next `/post` retries the same window. The first `/post` in a channel (no cursor) covers the whole conversation. *Why:* matches how teams actually work ("post the latest" repeatedly) and bounds Claude cost to the delta. The emoji-reaction trigger still summarizes its specific thread/window (not the rolling window). *Alternatives:* whole-channel-every-time — rejected (cost, re-summarizes old content); a fixed time/message count — rejected (arbitrary boundaries vs. the natural "since I last posted").
