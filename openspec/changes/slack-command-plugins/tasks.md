...
- [ ] 1.1 Define the slash command plugin contract in `Core/Slack/`: command name, description,
...
- [ ] 1.2 Define the context types passed to handlers (command invocation context and action context),
...
- [ ] 1.3 Define the handler result type used to deliver a message back through the response URL.
- [ ] 1.4 Add `EnabledCommands` (string array) to `SlackOptions`, matching the style of the existing
      `EligibleIssueTypes` / `ClosedStatuses` settings, with a doc comment stating that absence means
      disabled.

## 2. Registry and dispatch

- [ ] 2.1 Implement the registry: resolve a command by name, resolve an action's owning plugin by
      namespaced action id, and expose the registered set for manifest generation.
- [ ] 2.2 Filter registration by the `EnabledCommands` allow-list, so a plugin whose name is absent is
      not registered. Verify an empty or unset allow-list registers nothing.
- [ ] 2.3 Log at startup which plugins were discovered, which were registered, and which were skipped
      for not appearing in the allow-list, so a missing command is diagnosable.
- [ ] 2.4 Register plugins and the registry in `AddJiraSyncCore` in `ServiceCollectionExtensions.cs`.
- [ ] 2.5 Implement host-owned acknowledgement and background execution: acknowledge with the plugin's
...
- [ ] 2.6 Catch handler failures in the host and report a failure message to the invoking user, logging
      the exception.

## 3. Endpoint rewiring

- [ ] 3.1 Rewrite the `/slack/commands` handler to read the `command` field from the form payload and
...
- [ ] 3.2 Reply with an ephemeral "command is not available" message when the named command is not
      registered, invoking no handler.
- [ ] 3.3 Rewrite the `/slack/interactivity` handler to resolve the owning plugin from the namespaced
      action id, replacing the hardcoded confirm/reject ternary.
- [ ] 3.4 Refuse actions whose owning plugin is not registered, and actions with an unrecognized id,
      reporting that the action is no longer available rather than invoking a handler.

## 4. Relocate `/post`

- [ ] 4.1 Move the `/post` body from the endpoint lambda into a plugin type, as a pure relocation — no
...
- [ ] 4.2 Move the Confirm and Reject handlers onto the same plugin as its owned actions.
- [ ] 4.3 Change the action ids emitted by `CandidateBlocks` to the namespaced form, and update the
      handlers to match.
- [ ] 4.4 Verify behavioral parity with `post` present in the allow-list: same outcomes, same messages,
      same write-back side effects as before the move.

## 5. Manifest generation

- [ ] 5.1 Generate the manifest's `slash_commands` block from the registered command set, using each
      plugin's declared name and description.
- [ ] 5.2 Regenerate `docs/slack-app-manifest.yaml`, preserving its existing explanatory header
      comments, scopes, event subscriptions, and interactivity settings.
- [ ] 5.3 Add a test asserting the committed manifest matches what the registry would generate, so the
      two cannot drift.
- [ ] 5.4 Confirm the `commands` OAuth scope is unchanged by the command list, so updating commands is a
      manifest re-upload rather than an app reinstall, and note this in the manifest header comments.

## 6. Tests

- [ ] 6.1 Registry tests: command resolution by name, action resolution by namespaced id, and two
      plugins declaring the same bare action name resolving to different plugins.
- [ ] 6.2 Allow-list tests: a listed command registers, an unlisted command does not despite its plugin
      existing, an empty or unset allow-list registers nothing, and re-adding a name re-enables the
      command.
- [ ] 6.3 Dispatch tests: a signature-verified request routes to the owning plugin and no other, an
...
- [ ] 6.4 Interactivity tests: an action routes to its owning plugin, an action for an unregistered
      plugin is refused, and an unrecognized action id invokes nothing.
- [ ] 6.5 Acknowledgement tests: invocation is acknowledged with the plugin's text before the handler
...
- [ ] 6.6 Manifest tests: generated output lists exactly the registered commands, a disabled command is
      absent, and drift against the committed file fails.
- [ ] 6.7 Confirm ≥80% line coverage on the new and changed code before pushing, per the project's
...
- [ ] 7.4 Document the reversal procedure — add `"post"` to the allow-list, redeploy, regenerate and
...
- [ ] 8.1 Note in `slack-jira-actions`, `writeback-approval-gates`, and `work-item-assistant` that they
      should be built against the plugin contract rather than the endpoint.
- [ ] 8.2 Correct the stale claim in `openspec/changes/slack-jira-actions/design.md` that `IJiraClient`
