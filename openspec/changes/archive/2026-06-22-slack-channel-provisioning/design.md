...
- **Change signal mechanism: an `IWorkItemChangeListener` hook in the sync path.** The core's `WorkItemSyncService.ApplyIssueAsync` currently updates the mirror silently — it emits no events. We add a small seam: before saving an existing item, it computes whether the status or assignee changed and notifies registered `IWorkItemChangeListener`s (old → new). The Slack integration registers one listener that drives archive/unarchive and topic/context updates. *Why:* it's the minimal change to honor "driven by Jira events" without a message bus, and listeners are no-ops when Slack isn't configured. Listener calls are best-effort and must not fail the sync (a Slack outage can't block Jira mirroring). *Alternatives:* a full outbox/event table for Jira→Slack — deferred (heavier than needed at MDP volume); polling the mirror — rejected (latency + load).
- **Identity resolution is a pluggable resolver chain; config-map + invite-list ship first.** Jira Cloud hides user emails (GDPR), so an email→Slack lookup can't run there. Resolution is an ordered `IJiraSlackIdentityResolver` chain (DI `IEnumerable`, first non-null wins). Shipped now, needing **no enterprise API**: a **config map** (`Slack:UserMap`: Jira accountId/displayName → Slack id) to invite the assignee/reporter, and a fixed **`Slack:InviteUserIds`** list invited to every channel so it's visible even without resolution. Email-based resolvers drop into the same chain when an email source exists — **Jira Data Center** reads `emailAddress` off the issue; **Atlassian Cloud-org** uses the admin user-management API — then `ISlackClient.LookupUserIdByEmailAsync`. Unresolved → skip, never block provisioning. *Alternative:* hard-require email mapping — rejected (blocks provisioning on Cloud).
